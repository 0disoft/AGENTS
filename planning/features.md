...
> [!NOTE]
> 이 문서는 구현할 기능을 논의하기 위한 '기능 카탈로그'입니다. 이 목록을 바탕으로 프로젝트에 필요한 기능을 결정하세요. 최종 선택된 기능들의 요약은 `project-summary.md` 파일의 '핵심 기능' 섹션에 기록합니다. 기능을 선택하기 전에 `project-summary.md`를 참고하여 비즈니스 목표와 전체 맥락을 고려하세요.

아래 목록을 참고하여 구현할 페이지와 세부 기능을 구상합니다. [AI] 라벨이 붙은 항목은 AI가 판단하여 제안할 수 있는 기능 영역을 의미합니다. 최종 결정된 기능은 `project-summary.md`와 `development-plan.md`의 백로그에 반영됩니다.
...
페이지를 구성하는 데 필요한 구체적인 기능 아이디어 목록입니다. 예를 들어 '로그인 페이지'를 만들기로 했다면, '사용자 계정 및 인증' 목록에서 '로그인', '비밀번호 초기화', '소셜 로그인' 같은 세부 기능을 고려해볼 수 있습니다. 실제 백로그에 반영할 때에는 콜론(`:`) 뒷부분에 구현할 위치나 간단한 명세를 추가하여 사용합니다.
